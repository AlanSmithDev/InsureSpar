...
- **向量模型容错**：默认使用 `qwen3.7-text-embedding` 及其持久化文档向量；Qwen 不可用时自动切换到维度匹配的本地 MiniLM 索引。设置 `RAG_EMBEDDING_MODE=local` 可强制离线运行。
- **自适应检索上下文**：检索候选池与最终返回数量解耦；Qwen 根据语义置信度动态返回 2–4 条，MiniLM 降级模式扩大到 6 条并提高 BM25 权重。
...
完成下方环境变量、数据库和依赖准备后，在已激活后端 Python 环境的项目根目录执行：

```powershell
powershell -ExecutionPolicy Bypass -File scripts/dev.ps1
```

脚本固定后端默认端口为 `8001`，等待健康检查成功后才启动前端，并在退出时一并停止后端。若 Python 不在当前环境中，可使用 `-PythonPath` 指定解释器；端口也可通过 `-BackendPort` 和 `-FrontendPort` 覆盖。后端日志位于本地 `.dev-logs/`（已被 Git 忽略）。
...
```
后端服务启动成功后，可以首先访问 [http://127.0.0.1:8001/docs](http://127.0.0.1:8001/docs) 接口文档层确认所有服务加载无误（比如加载图谱与词向量文件）。
