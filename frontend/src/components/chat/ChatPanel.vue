...
import type { AppStatus, ChatMessage, Persona } from '../../types'
...
const latestTurn = computed(() => Math.max(0, ...props.messages.map(message => message.turn)))

function roundState(turn: number) {
  return turn === latestTurn.value ? props.status : 'completed'
}
  ...
  <div class="relative flex h-full min-w-0 flex-col overflow-hidden bg-white">
    <div ref="chatContainer" class="min-h-0 flex-1 overflow-y-auto px-4 py-6 sm:px-6 xl:px-8">
      ...
      <template v-for="group in groupedMessages" :key="group.turn">
        <RoundDivider v-if="group.turn > 0" :turn="group.turn" :state="roundState(group.turn)" />
        <div class="space-y-1">
          <ChatBubble
