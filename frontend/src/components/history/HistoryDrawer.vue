...
import { fetchHistorySessions, fetchHistoryDetail, deleteHistorySession } from '../../services/api'
...
const sessionToDelete = ref<HistorySession | null>(null)
...
// 删除会话相关函数
function confirmDeleteSession(session: HistorySession, event: Event) {
  event.stopPropagation() // 阻止事件冒泡，避免触发viewSession
...
async function executeDelete() {
  ...
  const sessionId = sessionToDelete.value.session_id
...
<template>
              ...
              >
                ...
                <div class="flex items-center justify-between mb-3">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <button
                      v-if="s.session_id !== currentSessionId"
                      type="button"
                      class="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-md hover:bg-red-50 text-[var(--color-text-muted)] hover:text-red-500"
                      title="删除此记录"
                      @click="confirmDeleteSession(s, $event)"
                    >
  ...
  <Teleport to="body">
    <Transition name="modal">
      ...
      >
        <div class="bg-white rounded-2xl shadow-2xl p-6 max-w-sm mx-4 animate-fade-in">
          <div class="flex items-center gap-3 mb-4">
            <div class="w-10 h-10 rounded-full bg-red-50 flex items-center justify-center">
              <Icon icon="lucide:alert-triangle" class="w-5 h-5 text-red-500" />
            </div>
            <div>
              <h3 class="text-[15px] font-semibold text-[var(--color-text-primary)]">确认删除</h3>
              <p class="text-[13px] text-[var(--color-text-secondary)]">此操作不可恢复</p>
            </div>
          </div>
          <p class="text-[13px] text-[var(--color-text-secondary)] mb-5">
            确定要删除与 <span class="font-medium text-[var(--color-text-primary)]">{{ sessionToDelete ? (getPersona(sessionToDelete.persona_id)?.name || sessionToDelete.persona_id) : '' }}</span> 的对练记录吗？删除后将无法恢复。
          </p>
          <div class="flex justify-end gap-2">
            <button
              type="button"
              class="px-4 py-2 text-[13px] font-medium rounded-lg border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:bg-[var(--color-surface)] transition-colors"
              @click="cancelDelete"
            >
              取消
            </button>
            <button
              type="button"
              class="px-4 py-2 text-[13px] font-medium rounded-lg bg-red-500 text-white hover:bg-red-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
