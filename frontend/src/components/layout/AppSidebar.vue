...
type View = 'home' | 'practice' | 'dashboard' | 'settings'
...
const navigation: Array<{ id: View; label: string; icon: string }> = [
  ...
  { id: 'practice', label: '实战对练', icon: 'lucide:message-circle' },
  { id: 'dashboard', label: '能力中心', icon: 'lucide:radar' },
]
...
<template>
  ...
  >
    <div class="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div :class="isCollapsed ? 'px-0' : 'px-3'">
        ...
        <button
          type="button"
          class="group flex h-12 items-center rounded-2xl text-[13px] text-[var(--color-text-secondary)] transition-[background-color,color] hover:bg-white/65 hover:text-[var(--color-text-primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]/30 mt-1"
          :class="isCollapsed ? 'mx-auto w-12 justify-center px-0' : 'w-full gap-3 px-4'"
          aria-label="设置"
          :title="isCollapsed ? '设置' : undefined"
          @click="$emit('navigate', 'settings')"
        >
          <Icon icon="lucide:settings" class="h-5 w-5 shrink-0 text-[var(--color-text-muted)] group-hover:text-[var(--color-accent)]" />
    ...
    >
      <Icon :icon="isCollapsed ? 'lucide:panel-left-open' : 'lucide:panel-left-close'" class="h-[18px] w-[18px]" />
    </button>
  </aside>
  ...
  <nav class="fixed inset-x-0 bottom-0 z-40 grid h-16 grid-cols-5 border-t border-[var(--color-border)] bg-white/95 px-2 backdrop-blur lg:hidden" aria-label="移动端导航">
    <button
      v-for="item in navigation"
      :key="item.id"
      type="button"
    ...
    <button
      type="button"
      class="flex flex-col items-center justify-center gap-1 text-[11px] font-medium"
      :class="currentView === 'settings' ? 'text-[var(--color-accent-dark)]' : 'text-[var(--color-text-muted)]'"
      @click="$emit('navigate', 'settings')"
    >
