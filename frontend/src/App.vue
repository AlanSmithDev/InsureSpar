...
// ── 状态 ──
const currentView = ref<'home' | 'practice' | 'dashboard' | 'settings'>('home')
...
function navigateTo(view: 'home' | 'practice' | 'dashboard' | 'settings') {
  if (view !== 'practice') {
...
function handleManualSSE(ev: SSEEvent, cid: string, turn: number, ensureCustomer: () => void) {
  switch (ev.type) {
    case 'status':
      ...
      break
    case 'tool_call':
      insertSysBefore(cid, '', turn, 'tool_call', ev.tool, ev.args)
      break
    case 'tool_result':
      insertSysBefore(cid, ev.content || '', turn, 'tool_result', ev.tool)
      break
    case 'token': {
      ensureCustomer()
...
function handleAutoSSE(ev: SSEEvent, sid: string, cid: string, turn: number, ensureSales: () => void, ensureCustomer: () => void) {
  switch (ev.type) {
    ...
    case 'sales_tool_call':
      insertSysBefore(sid, '', turn, 'tool_call', ev.tool, ev.args)
      break
    case 'sales_tool_result':
      insertSysBefore(sid, ev.content || '', turn, 'tool_result', ev.tool)
      break
    case 'sales_token': {
      ensureSales()
 ...
 * 辅助
 * ======================================== */
function addSys(content: string, turn: number, logType: string, toolName?: string, toolArgs?: string) {
  messages.value.push({ id: mid(), role: 'system', content, turn, logType: logType as any, toolName, toolArgs })
}
function insertSysBefore(beforeId: string, content: string, turn: number, logType: string, toolName?: string, toolArgs?: string) {
  const msg = { id: mid(), role: 'system' as const, content, turn, logType: logType as any, toolName, toolArgs }
