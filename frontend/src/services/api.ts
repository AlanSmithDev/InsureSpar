...
export async function deleteHistorySession(sessionId: string): Promise<{ success: boolean; message: string }> {
    const res = await fetch(`${BASE}/api/history/sessions/${sessionId}`, {
        method: 'DELETE',
    })
...
export async function fetchDashboardOverview(): Promise<DashboardOverview> {
    ...
    if (!res.ok) throw new Error(`获取训练概览失败: ${res.status}`)
...
export async function fetchDashboardCapabilities(): Promise<DashboardCapabilities> {
    ...
    if (!res.ok) throw new Error(`获取能力画像失败: ${res.status}`)
    const json = await res.json()
    return json.data || json
}
...
export async function fetchDashboardGrowth(period = 'last_10_sessions'): Promise<DashboardGrowth> {
    ...
    if (!res.ok) throw new Error(`获取成长趋势失败: ${res.status}`)
    const json = await res.json()
    return json.data || json
}
