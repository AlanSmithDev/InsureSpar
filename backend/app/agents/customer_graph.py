...
def customer_node(state: AgentState) -> dict:
    ...
    identity_anchor = build_identity_anchor(identity_contract, AgentRole.CUSTOMER)
...
【你的基本信息】
姓名：{identity_contract.customer.name}
人口属性：{persona.get('demographics', '未知')} | {persona.get('financial_status', '未知')} | {persona.get('health_status', '未知')}
...
重申：你是客户，你在回应销售对你说的话。"""
    ...
    # 工具调用的空文本无需校验；所有可见文本必须在进入 LangGraph checkpoint 前通过身份校验。
    latest_sales_message = next(
        (m.content for m in reversed(state["messages"]) if m.type == "human" and isinstance(m.content, str)),
        "",
    )
    validation = audit_identity_response(
        response.content if isinstance(response.content, str) else "",
        identity_contract,
        AgentRole.CUSTOMER,
        latest_counterpart_message=latest_sales_message,
    )
    if not validation.is_valid:
        print(f"⚠️ [客户Agent] 身份校验失败，重新生成: {validation.violations}")
        correction = SystemMessage(content=build_identity_correction_prompt(
            identity_contract, AgentRole.CUSTOMER, validation.violations
        ))
        response = llm.invoke(messages_to_send + [correction])
        validation = audit_identity_response(
            response.content if isinstance(response.content, str) else "",
            identity_contract,
            AgentRole.CUSTOMER,
        ...
        if not validation.is_valid:
            print(f"⛔ [客户Agent] 身份重试仍失败，阻断本轮输出: {validation.violations}")
            raise IdentityContractViolation(
                f"客户回复连续违反身份契约: {'；'.join(validation.violations)}"
            )
