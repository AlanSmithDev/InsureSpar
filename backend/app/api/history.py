...
@router.delete("/sessions/{session_id}", summary="删除会话及其所有关联数据")
def delete_session(session_id: str, db: Session = Depends(get_db)):
    """
    **功能**: 删除指定会话及其所有关联数据（对话记录、评分、终极报告）。
    **注意**: 此操作不可逆，请谨慎使用。
    """
    # 1. 检查会话是否存在
    ...
    try:
        # 2. 删除终极报告
        db.query(FinalReportRecord).filter(FinalReportRecord.session_id == session_id).delete()
        ...
        print(f"🗑️ [历史删除] 会话 {session_id} 及其所有关联数据已删除")
        return {"success": True, "message": f"会话 {session_id} 已删除"}

    except Exception as e:
        db.rollback()
        print(f"❌ [历史删除] 删除会话失败: {e}")
